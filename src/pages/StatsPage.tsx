        ...
        <div id="stats-view-content" className="flex-1 overflow-y-auto p-4 space-y-4">
            ...
            <div className="space-y-4">
                <button onClick={() => setModalState({ folderManagerOpen: true })} className="w-full bg-white p-4 rounded-xl shadow-sm border flex items-center justify-between group hover:bg-gray-50 transition-colors">
                    ...
                    <Folder size={16} className="text-gray-300 group-hover:text-gray-500" />
                </button>
                                ...
                                >
                                    {Object.entries(CITIES).map(([country, cities]) => (
                                        <optgroup key={country} label={country === 'China' ? t('setup.china', '中国') : t('setup.japan', '日本')}>
                                            {cities.map(city => {
                                                const isAvailable = country === 'Japan' || city.name === '北京' || city.name === '南京';
                                                return (
                                                    <option
                                                        key={city.name}
                                                    ...
                                                    >
                                                        {city.name}{!isAvailable ? ` (${t('common.comingSoon', 'TODO')})` : ''}
                                                    </option>
                                                );
                                            })}
                                        </optgroup>
