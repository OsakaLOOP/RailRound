...
export const FolderManagerModal: React.FC = () => {
    ...
    const saveUserFolders = (newFolders: any[]) => {
        ...
        if (user) {
            saveData(user.token, trips, pins, newFolders, badgeSettings).catch((e: any) => alert(translate('folder.saveFail', "保存失败: ") + e.message));
        }
    };
        ...
        <div className="fixed inset-0 z-[1000] bg-black/50 flex items-center justify-center p-4 animate-fade-in" onClick={onClose}>
            <div className="bg-white w-full max-w-md rounded-2xl shadow-2xl p-6 animate-slide-up" onClick={e => e.stopPropagation()}>
                <div className="flex justify-between items-center mb-6">
                    <h3 className="font-bold text-xl text-gray-800 flex items-center gap-2"><Folder size={24}/> {translate('folder.title', '收藏夹')}</h3>
                ...
                <form onSubmit={(e) => { e.preventDefault(); handleCreate(); }} className="flex gap-2 mb-4">
                    <input
                        ...
                        placeholder={translate('folder.placeholder', "名称...")}
                        value={newFolderName}
