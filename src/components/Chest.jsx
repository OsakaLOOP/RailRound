...
const Chest = ({ onDropItem } = {}) => {
    ...
    const triggerAutoRoute = () => {
        ...
        setTimeout(() => {
            ...
            const { setAutoForm, startEditingTrip } = store;
            const autoFormData = {
            ...
            setAutoForm(autoFormData);
            startEditingTrip(null, 'auto', autoFormData);
            toast.success(t('chest.openPlanner', 'Route candidates opened'));
            window.setTimeout(() => {
                window.dispatchEvent(new CustomEvent('trip-editor:auto-search-request'));
            }, 250);
