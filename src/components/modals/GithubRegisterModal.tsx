        ...
        <div className="fixed inset-0 z-[1000] bg-black/50 flex items-center justify-center p-4 animate-fade-in">
            <div className="bg-white w-full max-w-sm rounded-2xl shadow-2xl p-6 animate-slide-up">
                <div className="flex justify-between items-center mb-6">
                    <h3 className="font-bold text-xl text-gray-800 flex items-center gap-2"><Github size={24}/> {translate('githubReg.title', '完成注册')}</h3>
                    <button onClick={onClose}><X className="text-gray-400 hover:text-gray-600"/></button>
                </div>
                <div className="mb-4 text-sm text-gray-500">
                    {translate('githubReg.desc', '欢迎使用 GitHub 登录！请设置您的用户名和密码以完成账户创建。')}
                </div>
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                    <div>
                        <label className="block text-xs font-bold text-gray-500 mb-1">{translate('githubReg.username', '用户名')}</label>
                    ...
                    <div>
                        <label className="block text-xs font-bold text-gray-500 mb-1">{translate('githubReg.password', '密码')}</label>
                    ...
                    <button type="submit" disabled={loading} className="w-full bg-gray-900 text-white py-3 rounded-lg font-bold hover:bg-black transition-colors disabled:opacity-50 flex justify-center">
                        {loading ? <Loader2 className="animate-spin"/> : translate('githubReg.createBtn', '创建账户')}
                    </button>
                </form>
            </div>
        </div>
