...
import { X, Map as MapIcon, Building2, Train, Search, MapPin, ListFilter } from 'lucide-react';
import { useStore } from '../../store';
import { buildLineSelectorGroups, CategoryKey } from '../../utils/lineSelectorBuilder';
...
export const StationLineSearchModal: React.FC<Props> = ({ isOpen, initialMode, onClose, onSelect, allowedLines }) => {
    ...
    const groupsData = useMemo(() => buildLineSelectorGroups(railwayData, allowedLines, priorityRegion), [railwayData, allowedLines, priorityRegion]);

    const tabStats = useMemo(() => {
        const stats: Record<string, Record<string, { companies: number; lines: number }>> = {};
        Object.entries(groupsData).forEach(([tab, regions]) => {
            const countrySets: Record<string, { companies: Set<string>; lines: Set<string> }> = {};
            regions.forEach(r => {
                const country = r.name === '中国大陆' ? 'CN' : 'JP';
                if (!countrySets[country]) countrySets[country] = { companies: new Set(), lines: new Set() };
            ...
            Object.entries(countrySets).forEach(([country, s]) => {
                stats[tab][country] = { companies: s.companies.size, lines: s.lines.size };
            });
        });
        return stats;
    }, [groupsData]);
    ...
    const results = useMemo(() => {
        ...
        for (const lineKey in railwayData) {
            ...
            // Stop processing if we have enough results for both arrays
            if (matchedLines.length >= 50 && matchedStations.length >= 100) break;

            const lineData = railwayData[lineKey];

            // Avoid string split/join allocations
        ...
        <div className="fixed inset-0 z-[700] flex flex-col items-center justify-end md:justify-center p-0 md:p-4">
            ...
            >
                <div className="relative w-full h-full flex flex-col">
                    ...
                    {/* ===== LINE SELECTOR LAYER ===== */}
                    <div className={`absolute inset-0 flex flex-col bg-white transition-opacity duration-300 ${mode === 'line' ? 'opacity-100 pointer-events-auto z-10' : 'opacity-0 pointer-events-none z-0'}`}>
                            ...
                            {(['JR', 'Private', 'City'] as CategoryKey[]).map(tab => (
                                ...
                                >
                                    <span className="text-sm font-bold leading-tight">
                                        {tab === 'JR' ? t('lineSel.jr', 'JR 集団') : tab === 'Private' ? t('lineSel.private', '私鉄・第三セクター') : t('lineSel.subway', '地下鉄・新交通')}
                                    </span>
                                    <div className="flex flex-wrap justify-center gap-x-2 gap-y-0.5 opacity-90 scale-90 mt-0.5">
                                        {Object.entries(tabStats[tab] || {})
                                            .sort(([a], [b]) => {
                                                const priority = priorityRegion === '中国大陆' ? 'CN' : 'JP';
                    ...
                    {/* ===== GLOBAL SEARCH LAYER ===== */}
                    <div className={`absolute inset-0 flex flex-col bg-white transition-opacity duration-300 ${mode === 'search' ? 'opacity-100 pointer-events-auto z-10' : 'opacity-0 pointer-events-none z-0'}`}>
                        <div className="p-4 border-b bg-white flex items-center gap-3 shrink-0 sticky top-0 z-20 shadow-sm">
                            ...
                            <button onClick={onClose} className="p-2 ml-1 hover:bg-gray-100 rounded-full text-gray-500 transition shrink-0" title={t('common.close', '关闭')}>
                                <X size={20} />
                            </button>
                        </div>
                                        ...
                                        <div>
                                            ...
                                            <div className="bg-white rounded-xl border border-gray-200 overflow-hidden shadow-sm divide-y divide-gray-50">
                                                {results.stations.map((station, idx) => (
                                                    <button
                                                        key={`station-${idx}`}
                                                        onClick={() => onSelect(station.lineKey, station.stationId)}
