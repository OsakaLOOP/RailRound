...
## 2024-05-18 - [Replace turf.length with Native Haversine]
**Learning:** Found an opportunity to replace `turf.length(turf.lineString(...))` with a native `calcPolylineDist(coords)` helper. Turf.js `lineString` generation paired with mapping `[lat, lng]` to `[lng, lat]` creates massive array allocations and garbage collection pressure when running repetitively (e.g., rendering trip stats).
**Action:** Replaced Turf.js usage for basic polyline distances with an exported O(N) `calcPolylineDist` wrapper around `calcDist` in `tripCalculator.js`, achieving ~10x speedup for polyline measurements in testing.
