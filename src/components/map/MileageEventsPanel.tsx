...
export const MileageEventsPanel: React.FC = () => {
  ...
  if (!open) {
    return (
      <button
        className="rl-map-control rl-focus absolute right-4 top-4 z-[410] flex items-center justify-center transition-all active:scale-95"
  ...
  return (
    <section className="rl-modal-panel absolute right-4 top-4 z-[410] flex max-h-[calc(100dvh-8rem)] w-[min(24rem,calc(100vw-2rem))] flex-col overflow-hidden">
      <header className="rl-modal-header flex items-center justify-between px-3 py-2">
        <div className="flex items-center gap-2 text-sm font-semibold text-slate-800">
          <MapPinned size={17} className="text-teal-600" />
          {t("mileageEvents.title", "Mileage events")}
        </div>
        <button
          className="rl-close-button"
      ...
      <div className="rl-segmented mx-3 mt-3 flex gap-1">
        {[
          ...
          <button
            ...
            className={`flex flex-1 items-center justify-center gap-1 rounded-md px-2 py-1.5 text-xs font-semibold transition-colors ${
              mode === key ? "bg-white text-teal-700 shadow-sm" : "text-slate-500 hover:bg-white/70 hover:text-slate-700"
            }`}
          >
            <Icon size={14} />
            {label}
      ...
      <div className="space-y-2 border-b border-slate-200 p-3">
        ...
        />
        <textarea
          className="rl-input min-h-16 resize-none"
        ...
        <div className="grid grid-cols-2 gap-2">
          <button
            className="rl-primary-action flex items-center justify-center gap-1 px-2 py-2 text-xs disabled:bg-slate-300"
          ...
          <button
            className="rl-secondary-action flex items-center justify-center gap-1 px-2 py-2 text-xs disabled:bg-slate-100 disabled:text-slate-400"
      ...
      <div className="min-h-0 flex-1 overflow-auto p-2">
        {visibleEvents.length === 0 ? (
          <div className="rl-card-muted border-dashed p-3 text-center text-xs text-slate-500">
            {t("mileageEvents.empty", "没有匹配的里程事件")}
          </div>
        ) : (
          <div className="space-y-2">
            {visibleEvents.map((entry) => (
              <article key={`${entry.event.id}:${entry.distanceMetersFromRunStart}`} className="rl-card-muted p-2">
                <div className="flex items-start justify-between gap-2">
                  ...
                  <button
                    className="rl-icon-button !min-h-8 !min-w-8 text-slate-400 hover:bg-red-50 hover:text-red-600"
                    onClick={() => removeEvent(entry.event.id)}
                    title={t("common.deleteConfirm", "确认删除?")}
                    aria-label={t("common.deleteConfirm", "确认删除?")}
                  >
                    <Trash2 size={15} />
                  </button>
                </div>
