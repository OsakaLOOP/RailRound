...
// O(N) utility to compute path distance over an array of [lng, lat] coords without Turf allocations
export const calcPolylineDist = (coords) => {
    if (!coords || !coords.length) return 0;
    let dist = 0;
    for (let i = 1; i < coords.length; i++) {
        dist += calcDist(coords[i-1][0], coords[i-1][1], coords[i][0], coords[i][1]);
    }
    return dist;
};
