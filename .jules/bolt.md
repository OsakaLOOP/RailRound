...
## 2024-05-18 - [Optimization of turf.length distance calculation in tripCalculator]
**Learning:** Found an opportunity to replace `turf.length(turf.lineString(...))` which parses geojson overheads to O(N) by using mathematical coordinate calculation like haversine formula (`calcDist`) over multiple polylines, bringing the overall performance from ~1.3ms to ~0.12ms locally without accuracy tradeoff.
**Action:** Created `calcPolylineDist` wrapper to loop through coordinates array directly using mathematical calculation `calcDist`, replacing multiple turf library wrappers.
