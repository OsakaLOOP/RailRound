...
5. 现有 src/rail-graph-v1-mvp/ 文件不修改, 但允许 import 其纯函数 (rule-handlers, spatial-helpers 等) 和**复刻**其核心逻辑 (compileTopology, clean pipeline) 到新目录; 新 Aggregate UI 走新入口 rail-graph-aggregate.html
...
- **ServicePattern**: 通过交互式 IntentionChain 编辑器构造, 路径导出后命名/着色/持久化。支持直通运転 (乗り入れ) 场景 — 在合并 topology 上直接寻路即可跨越多条线路
- **乘客换乘查询 (Cross-Pattern Route Query)**: 给定已保存的多条独立 ServicePattern, 查询从 A 站到 B 站需要坐哪几条 pattern (乘客视角)
...
- **导入时**: 从 N 个 member workspace 读取 source features, 一次性应用各自的 clean pipeline (rule + override两方面, 不可缺失), 合并去重后写入 aggregate 自己的存储
...
- `fixtures/aggregate-東北本線_v2.cleaned.geojson` (東北本線\_v2, 300 features)
...
└── src/rail-graph-aggregate/app.ts             (几乎完整复刻 mvp/app.ts 的核心逻辑: compileTopology, clean pipeline, map/list view 等)
    ├── aggregate-state.ts                      (state + import member workspaces + 持久化 aggregate 数据)
    ├── service-pattern/
...
| **PR2** | 乘客换乘查询 (Cross-Pattern Route Query: transfer graph + two-tier dijkstra + UI)    | ~4-5         | `npm run rail:aggregate:verify:cross-pf` |
...
- 原 mvp 的workspace右边菜单功能基本保留或者进一步适应性改造. workflow 则不需要迁移.
...
| `src/rail-graph-aggregate/aggregate-state.ts`              | 新建                   | 持久化 aggregate 数据; 暴露 `loadAggregate()` (纯 fs 读, Node 兼容) + `importWorkspaces()` (浏览器端导入动作)                      |
...
- **app.ts 是复刻不是复用**: aggregate 的 `app.ts` 几乎完整复刻 `src/rail-graph-v1-mvp/app.ts` 的核心逻辑 (compileTopology, clean pipeline, fidOf, map/list view binding 等)。不是 import 调用 MVP app.ts (它有大量 DOM 全局状态不可分离), 而是把核心纯逻辑复制过来, 适配 aggregate 的多 workspace 场景。workflow steps (prepare/clean/annotate/compile/validate/export) 不需要迁移。
...
- **localStorage 共享**: `rail-graph-aggregate.html` 与 `rail-graph-mvp.html` 在同一 origin (同一 vite dev server), 共享 localStorage。aggregate app 可直接读 MVP 的 `railround:mvp:workspace-state:v1` 获取已有 workspace 列表及其 `MvpProjectState` (含 lineDir, overridePath, scriptsRoot 等路径)。导入 UI 应展示这些 workspace 供用户勾选, 无需用户手动输入路径。
- **导入时数据定位**: 每个 member workspace 的 `project.lineDir` → `matched_assets.geojson`; `project.overridePath` → override JSON; `project.scriptsRoot + "\\filter_rules.json"` → 规则文件。全部通过 server API (`readPipelineArtifact` / `readOverrides`) 读取。
- **导入前提: 源 workspace 必须已完成 annotate**: 未标注的 workspace 导入后 compileTopology 会报 error (如東北本線\_v2 当前状态)。导入 UI 应检查源 workspace 的 progress.annotate.status, 未完成时警告用户。
- **workspace key 格式**: MVP 中 `workspaceKey()` = `"${companyName}::${lineName}"` (如 `"東日本旅客鉄道::仙石線"`)。aggregate 的 `memberWorkspaceKeys` 使用同一格式。`loadAggregate(aggregateKey)` 用 aggregateKey (如 `"senseki-tohoku"`) 定位已持久化的 aggregate 数据, 不需要再传 workspace key 列表。
- **verify 脚本的 Node 兼容性**: verify 骨架 import `aggregate-state.ts`。`loadAggregate` 必须可在 Node (tsx) 中运行 — 读已持久化的 aggregate 数据 (纯 fs 读), 不依赖 localStorage 或 fetch。`importWorkspaces` 是浏览器端动作, verify 不调用它。
...
- **持久化**: aggregate 的 features, topology, service-patterns, user-events 等数据的持久化 schema 和路径由 agent 自行设计。要求: (1) 通过 server API 读写 (不用 localStorage 存大数据); (2) `loadAggregate` 在 Node (tsx) 中可纯 fs 读取 (verify 兼容); (3) 多个 aggregate 实例互不干扰 (用 aggregateKey 区分)
...
1. `loadAggregate(aggregateKey)` 成功返回, `topo.edges.length > 0` 且 ≥ 各 ws 之和的 80% (允许去重)
...
**Pattern 数据来源**: verify 脚本调用 `loadAggregate` 读取已持久化的 aggregate 数据 (含 patterns)。首次跑 verify 前, 用户需在 aggregate UI 中至少创建 1 条 ServicePattern 并保存。verify 不自己构造 pattern, 也不调用 `importWorkspaces`。

**报告位置**: `src/rail-graph-aggregate/.verify/patterns-*.json` (脚本自动写)

---

## 5. PR 2 · 乘客换乘查询 (Cross-Pattern Route Query)

### 5.1 目标

- 给定 (fromStation, toStation), 在已保存的 ServicePattern 集合上查询乘客需要坐哪几条 pattern 才能到达目的地
- **注意**: 这不是直通运行寻路 (直通 pattern 在 PR1 中通过 IntentionChain 直接创建, 单条 pattern 跨越多条线路)。本 PR 处理乘客视角: 已有多条独立 pattern, 查询换乘方案
...
- 输出: 换乘路径 = `[{ patternRef, edgeSequence, stationSequence, transferStation? }]`
- UI: aggregate app.ts 加 "Route Query" 表单 (from / to station picker); 选定后高亮地图
...
| `src/rail-graph-aggregate/app.ts`                           | 修改                   | 加 route query UI 面板 + 调用 resolver + 高亮                                    |
...
- **与直通运転的区别**: 直通 pattern (如仙石東北ライン) 在 PR1 中作为单条 pattern 创建, 不经过本模块。本模块处理的是"用户有多条独立 pattern, 需要换乘才能到达"的场景
...
1. `transfer-graph.buildTransferGraph(patterns)` 输出至少 1 条 transfer relation (即两条独立 pattern 通过共享 station 联通)
2. `resolver.resolveCrossPattern({patterns, from: <pattern_A 某站>, to: <pattern_B 某站>})` 返回路径含 ≥ 2 `PatternHop` (说明确实需要换乘)
3. Transfer station 在两个 PatternHop 的 traceSequence 中都能找到
4. 总 edgeCount = Σ hop.edgeSequence.length, 且合理 (sanity: 跨线路径 edgeCount ≤ aggregate.edges.length)
5. 反向场景: from/to 互换, 输出对称路径 (edgeCount 一致, pattern 序列反转)
6. PR1 verify 仍 PASS

**注意**: 测试换乘需要至少 2 条独立 ServicePattern (非直通)。如果当前只有 1 条直通 pattern (仙石東北ライン), 需要额外创建独立的東北本線 pattern 和仙石線 pattern 作为测试数据。
...
- `tsx` 入口, Node 跑, **不开浏览器**
- 调用 `loadAggregate(aggregateKey)` 读取已持久化的 aggregate 数据 (纯 fs 读, 不依赖 server/localStorage)
...
## 8. Out of Scope (本 goal 不修改以下文件, 但允许 import 其导出)

- `src/rail-graph-v1-mvp/app.ts`, `map-view.ts`, `list-view.ts` 等 MVP shell 代码 (不修改; 可复刻其逻辑到新目录)
- `src/rail-graph-v1-mvp/rule-handlers.ts`, `spatial-helpers.ts` (不修改; 可直接 import 使用)
...
| ------------- | -------------- | ------------------------------ | --------------------------------------- |
| 仙台上行起始  | way/1015018069 | `way:1015018069:rail:東北本線` | 東北本線\_v2                            |
| 仙台下行终到  | way/884011779  | `way:884011779:rail:東北本線`  | 東北本線\_v2                            |
