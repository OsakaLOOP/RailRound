...
## 2024-06-17 - [Optimized Path Distance Computation]
**Learning:** Found an opportunity to replace `turf.length(turf.lineString(...))` with a native $O(N)$ Haversine calculation (`calcPolylineDist`) when computing aggregate distances across large coordinate arrays. The original method caused massive overhead due to temporary object allocations (e.g., mapping arrays and creating GeoJSON instances).
**Action:** Always prefer the manual, single-pass `calcPolylineDist(coords)` over Turf.js methods for distance accumulation across raw coordinate arrays to minimize garbage collection pressure and prevent linear slowdowns.
