...
const Chest = ({ onDropItem } = {}) => {
    ...
    const triggerAutoRoute = () => {
        ...
        setTimeout(() => {
            ...
            const { setAutoForm, setModalState, setEditorMode, railwayData, railGraphRuntime, addTrip, isAprilFool, setTripForm } = store;
            setAutoForm({
            ...
            if (railwayData) {
                ...
                setTimeout(() => {
                    const result = planAppRoute({
                        startLineKey: ticketStart.lineKey,
                        startStationId: ticketStart.id,
                        endLineKey: ticketEnd.lineKey,
                        endStationId: ticketEnd.id,
                        railwayData,
                        railGraphRuntime,
                        maxTransfersOverride: isAprilFool ? -1 : 6
                    });

                    if (result.status === 'ok' && result.segments && result.segments.length > 0) {
                        toast.success(t('chest.routeFound', '找到路径'), { id: toastId });
                        const newTrip = result.source === 'rail_graph'
                            ? {
                                ...tripResultToLegacyTrip(result.trip.tripResult, result.trip.runtimeArtifacts),
                                id: Date.now(),
